...
void Plane::draw(std::shared_ptr<SceneContext> ctx, glm::mat4x4 model) {
    ...
    glUniformMatrix4fv(m_program->uniforms_location["camera"], 1, GL_FALSE, glm::value_ptr(ctx->perspective));
    glUniformMatrix4fv(m_program->uniforms_location["world"], 1, GL_FALSE, glm::value_ptr(ctx->activeCamera->view()));
